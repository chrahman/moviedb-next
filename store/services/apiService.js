...
export const getAllDiscovers = async () => {
  const data = await axios.get(`${API_BASE_URL}/discover/movie?api_key=${key}`);
  return data.data;
};

//Upcomming Movies API
export const getAllUpcomings = async () => {
  const data = await axios.get(`${API_BASE_URL}/movie/upcoming?api_key=${key}`);
  return data.data;
};

//Trendings Movies API
export const getAllTrendings = async () => {
  const data = await axios.get(`${API_BASE_URL}/trending/all/day?api_key=${key}`);
  return data.data;
};

//TVShows API
export const getAllTvShows = async () => {
  const data = await axios.get(`${API_BASE_URL}/tv/popular?api_key=${key}`);
  return data.data;
};

//People API
export const getAllPeoples = async () => {
  const data = await axios.get(`${API_BASE_URL}/person/popular?api_key=${key}`);
  return data.data;
};
